...
var initializeGitandNpm = function initializeGitandNpm(project_name, params) {
  ...
  return "cd ".concat(project_name, " && git init && touch .gitignore .env.shared .env && echo \"").concat(params.gitignore, "\" >> .gitignore && npm init -y");
};
...
var installEs6MongoDependencies = function installEs6MongoDependencies(project_name, params) {
  ...
  return "cd ".concat(project_name, " && npm install ").concat(params.dependencies, " --save && npm install ").concat(params.devDependencies, " --save-dev  && touch .babelrc && echo \"").concat(JSON.stringify(params.babel), "\" >> .babelrc && echo \"").concat(params.envConfig, "\" >> .env.shared && echo \"").concat(params.envConfig, "\" >> .env ");
};
...
var createSequelizeProjectFolders = function createSequelizeProjectFolders(project_name) {
  ...
  return "mkdir ".concat(project_name, " && cd ").concat(project_name, " && touch index.js  && mkdir controllers && mkdir routes");
};
...
var initSequelizeCli = function initSequelizeCli(project_name) {
  ...
  return "cd ".concat(project_name, " && npx sequelize-cli init");
};
...
var insertSequelizeServerConfig = function insertSequelizeServerConfig(project_name, params) {
  ...
  return "cd ".concat(project_name, " && echo \"").concat(params.serverConfig, "\" >> index.js");
};
...
var initializeSequelizeControllers = function initializeSequelizeControllers(project_name, params) {
  ...
  return "cd ".concat(project_name, " && cd controllers && touch SampleController.js && echo \"").concat(params.controller, "\" >> SampleController.js");
};
...
var initializeSequelizeRouters = function initializeSequelizeRouters(project_name, params) {
  console.info('Creating routers');
  return "cd ".concat(project_name, " && cd routes && touch SampleRouter.js index.js && echo \"").concat(params.routerModelTemplate, "\" >> SampleRouter.js  && echo \"").concat(params.routerTemplate, "\" >> index.js");
};
/* Postgres Commands*/
